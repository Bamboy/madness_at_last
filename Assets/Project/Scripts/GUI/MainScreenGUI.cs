...
public class MainScreenGUI : MonoBehaviour {
	...
	string width;
	string height;
	Vector2 ResolutionPosition = Vector2.zero;
	Vector2 GraphicsPosition = Vector2.zero;
	...
	void Awake(){
		...
		KeyToggle = new bool[KeyManager.keyCodes.Count];
	}
	void OnGUI(){
	...
	void OptionsButton(){
		...
		if(OptionToggle){
			...
			case 4://controls
				...
				GUI.Label(new Rect(100, 30, 100, 50), "Forward: " + KeyManager.GetKeyCode("forward").ToString());
				GUI.Label(new Rect(100, 60, 100, 50), "Back: " + KeyManager.GetKeyCode("back").ToString());
				GUI.Label(new Rect(100, 90, 100, 50), "Left: " + KeyManager.GetKeyCode("left").ToString());
				GUI.Label(new Rect(100, 120, 100, 50), "Right: " + KeyManager.GetKeyCode("right").ToString());
				GUI.Label(new Rect(100, 150, 100, 50), "Jump: " + KeyManager.GetKeyCode("jump").ToString());
				GUI.Label(new Rect(100, 180, 100, 50), "Run: " + KeyManager.GetKeyCode("run").ToString("g"));
				GUI.Label(new Rect(100, 210, 100, 50), "Crouch: " + KeyManager.GetKeyCode("crouch").ToString("G"));
				for(int i = 0; i < KeyManager.keyCodes.Count; i++){
					KeyToggle[i] = GUI.Toggle(new Rect(180, 30 * (i + 1), 15, 15), KeyToggle[i], "");
					if(KeyToggle[i]){
						Event e = Event.current;
						if(e.isKey){
							if(KeyToggle[0]){
								KeyManager.EditKeyCode("forward", e.keyCode);
								KeyToggle[0] = false;
							}
							if(KeyToggle[1]){
								KeyManager.EditKeyCode("back", e.keyCode);
								KeyToggle[1] = false;
							}
							if(KeyToggle[2]){
								KeyManager.EditKeyCode("left", e.keyCode);
								KeyToggle[2] = false;
							}
							if(KeyToggle[3]){
								KeyManager.EditKeyCode("right", e.keyCode);
								KeyToggle[3] = false;
							}
							if(KeyToggle[4]){
								KeyManager.EditKeyCode("jump", e.keyCode);
								KeyToggle[4] = false;
							}
							if(KeyToggle[5]){
								KeyManager.EditKeyCode("run", e.keyCode);
								KeyToggle[5] = false;
							}
							if(KeyToggle[6]){
								KeyManager.EditKeyCode("crouch", e.keyCode);
								KeyToggle[6] = false;
							}
						}
					}
